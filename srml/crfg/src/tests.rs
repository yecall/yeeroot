...
use crate::mock::{Crfg, System, new_test_ext};
...
fn authorities_change_logged() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		...
		Crfg::schedule_change(vec![(4, 1), (5, 1), (6, 1)], 0, None).unwrap();
...
fn authorities_change_logged_after_delay() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		System::initialize(&1, &Default::default(), &Default::default());
		Crfg::schedule_change(vec![(4, 1), (5, 1), (6, 1)], 1, None).unwrap();
		Crfg::on_finalize(1);
		let header = System::finalize();
		assert_eq!(header.digest, testing::Digest {
			logs: vec![
...
fn cannot_schedule_change_when_one_pending() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		System::initialize(&1, &Default::default(), &Default::default());
		Crfg::schedule_change(vec![(4, 1), (5, 1), (6, 1)], 1, None).unwrap();
		assert!(Crfg::pending_change().is_some());
		assert!(Crfg::schedule_change(vec![(5, 1)], 1, None).is_err());
		...
		assert!(Crfg::pending_change().is_none());
		assert!(Crfg::schedule_change(vec![(5, 1)], 1, None).is_ok());
...
fn dispatch_forced_change() {
	with_externalities(&mut new_test_ext(vec![(1, 1), (2, 1), (3, 1)]), || {
		...
		assert!(Crfg::pending_change().is_some());
		assert!(Crfg::schedule_change(vec![(5, 1)], 1, Some(0)).is_err());
		...
		for i in 2..7 {
			...
			assert!(Crfg::pending_change().unwrap().forced.is_some());
			assert_eq!(Crfg::next_forced(), Some(11));
			assert!(Crfg::schedule_change(vec![(5, 1)], 1, None).is_err());
			assert!(Crfg::schedule_change(vec![(5, 1)], 1, Some(0)).is_err());
		...
		// add a normal change.
		{
			...
			assert!(Crfg::pending_change().is_none());
			assert_eq!(Crfg::crfg_authorities(), vec![(4, 1), (5, 1), (6, 1)]);
			assert!(Crfg::schedule_change(vec![(5, 1)], 1, None).is_ok());
			Crfg::on_finalize(7);
			header = System::finalize();
		}
		...
		for i in 9..11 {
			System::initialize(&i, &header.hash(), &Default::default());
			assert!(Crfg::pending_change().is_none());
			assert_eq!(Crfg::crfg_authorities(), vec![(5, 1)]);
			assert_eq!(Crfg::next_forced(), Some(11));
			assert!(Crfg::schedule_change(vec![(5, 1), (6, 1)], 5, Some(0)).is_err());
			Crfg::on_finalize(i);
			header = System::finalize();
		}

		{
			...
			assert!(Crfg::pending_change().is_none());
			assert!(Crfg::schedule_change(vec![(5, 1), (6, 1), (7, 1)], 5, Some(0)).is_ok());
			assert_eq!(Crfg::next_forced(), Some(21));
			Crfg::on_finalize(11);
			header = System::finalize();
		}
		let _ = header;
