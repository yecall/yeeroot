...
impl<TMessage, TSubstream> CustomProto<TMessage, TSubstream> {
	...
	pub fn disconnect_peer(&mut self, peer_id: &PeerId) {
		debug!(target: "sub-libp2p-foreign", "External API => Disconnect {:?}", peer_id);
	...
	fn disconnect_peer_inner(&mut self, peer_id: &PeerId, ban: Option<Duration>) {
		...
		match mem::replace(entry.get_mut(), PeerState::Poisoned) {
			...
			PeerState::DisabledPendingEnable { open, connected_point, timer } => {
				debug!(target: "sub-libp2p-foreign", "PSM <= Dropped({:?})", peer_id);
			...
			PeerState::Enabled { open, connected_point } => {
				debug!(target: "sub-libp2p-foreign", "PSM <= Dropped({:?})", peer_id);
				self.peerset.dropped(peer_id.clone());
				debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Disable", peer_id);
			...
			PeerState::Incoming { connected_point, .. } => {
				let inc = if let Some(inc) = self.incoming.iter_mut()
					...
					error!(target: "sub-libp2p-foreign", "State mismatch in libp2p: no entry in \
			...
			PeerState::Poisoned =>
				error!(target: "sub-libp2p-foreign", "State of {:?} is poisoned", peer_id),
		}
	}
	...
	pub fn send_packet(&mut self, target: &PeerId, message: TMessage) {
		...
		trace!(target: "sub-libp2p-foreign", "External API => Packet for {:?}", target);
		trace!(target: "sub-libp2p-foreign", "Handler({:?}) <= Packet", target);
		self.events.push(NetworkBehaviourAction::SendEvent {
	...
	fn peerset_report_connect(&mut self, peer_id: PeerId) {
		let mut occ_entry = match self.peers.entry(peer_id) {
			...
			Entry::Vacant(entry) => {
				...
				debug!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Starting to connect", entry.key());
				debug!(target: "sub-libp2p-foreign", "Libp2p <= Dial {:?}", entry.key());
		...
		match mem::replace(occ_entry.get_mut(), PeerState::Poisoned) {
			PeerState::Banned { ref until } if *until > Instant::now() => {
				debug!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Will start to connect at \
			...
			PeerState::Banned { .. } => {
				debug!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Starting to connect", occ_entry.key());
				debug!(target: "sub-libp2p-foreign", "Libp2p <= Dial {:?}", occ_entry.key());
			...
			PeerState::Disabled { open, ref connected_point, banned_until: Some(ref banned) }
				...
				debug!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Has idle connection through \
			...
			PeerState::Disabled { open, connected_point, banned_until: _ } => {
				debug!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Enabling previously-idle \
					connection through {:?}", occ_entry.key(), connected_point);
				debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Enable", occ_entry.key());
				self.events.push(NetworkBehaviourAction::SendEvent {
			...
			PeerState::Incoming { connected_point, .. } => {
				debug!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Enabling incoming \
					connection through {:?}", occ_entry.key(), connected_point);
			...
			st @ PeerState::Enabled { .. } => {
				warn!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Already connected to this \
					peer", occ_entry.key());
				*occ_entry.into_mut() = st;
			},
			st @ PeerState::DisabledPendingEnable { .. } => {
				warn!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Already have an idle \
					connection to this peer and waiting to enable it", occ_entry.key());
				*occ_entry.into_mut() = st;
			},
			st @ PeerState::Requested { .. } | st @ PeerState::PendingRequest { .. } => {
				warn!(target: "sub-libp2p-foreign", "PSM => Connect({:?}): Received a previous \
					request for that peer", occ_entry.key());
				*occ_entry.into_mut() = st;
			},

			PeerState::Poisoned =>
				error!(target: "sub-libp2p-foreign", "State of {:?} is poisoned", occ_entry.key()),
		}
	}
	...
	fn peerset_report_disconnect(&mut self, peer_id: PeerId) {
		let mut entry = match self.peers.entry(peer_id) {
			Entry::Occupied(entry) => entry,
			Entry::Vacant(entry) => {
				debug!(target: "sub-libp2p-foreign", "PSM => Drop({:?}): Node already disabled", entry.key());
				return
			}
		};
		...
		match mem::replace(entry.get_mut(), PeerState::Poisoned) {
			...
			PeerState::DisabledPendingEnable { open, connected_point, timer } => {
				debug!(target: "sub-libp2p-foreign", "PSM => Drop({:?}): Interrupting pending \
					enable", entry.key());
			...
			PeerState::Enabled { open, connected_point } => {
				debug!(target: "sub-libp2p-foreign", "PSM => Drop({:?}): Disabling connection", entry.key());
				debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Disable", entry.key());
				self.events.push(NetworkBehaviourAction::SendEvent {
			...
			st @ PeerState::Incoming { .. } => {
				error!(target: "sub-libp2p-foreign", "PSM => Drop({:?}): Was in incoming mode",
					entry.key());
				*entry.into_mut() = st;
			},
			PeerState::Requested => {
				...
				debug!(target: "sub-libp2p-foreign", "PSM => Drop({:?}): Was not yet connected", entry.key());
				entry.remove();
			},
			...
			PeerState::Poisoned =>
				error!(target: "sub-libp2p-foreign", "State of {:?} is poisoned", entry.key()),
		}
	}
	...
	fn peerset_report_accept(&mut self, index: substrate_peerset::IncomingIndex) {
		let incoming = if let Some(pos) = self.incoming.iter().position(|i| i.incoming_id == index) {
			...
			error!(target: "sub-libp2p-foreign", "PSM => Accept({:?}): Invalid index", index);
			return
		};

		if !incoming.alive {
			debug!(target: "sub-libp2p-foreign", "PSM => Accept({:?}, {:?}): Obsolete incoming,
				sending back dropped", index, incoming.peer_id);
			debug!(target: "sub-libp2p-foreign", "PSM <= Dropped({:?})", incoming.peer_id);
		...
		let state = if let Some(state) = self.peers.get_mut(&incoming.peer_id) {
			...
			error!(target: "sub-libp2p-foreign", "State mismatch in libp2p: no entry in peers \
		...
		let connected_point = if let PeerState::Incoming { connected_point } = state {
			...
			error!(target: "sub-libp2p-foreign", "State mismatch in libp2p: entry in peers corresponding \
				to an alive incoming is not in incoming state");
			return
		};

		debug!(target: "sub-libp2p-foreign", "PSM => Accept({:?}, {:?}): Enabling connection \
			through {:?}", index, incoming.peer_id, connected_point);
		debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Enable", incoming.peer_id);
		self.events.push(NetworkBehaviourAction::SendEvent {
	...
	fn peerset_report_reject(&mut self, index: substrate_peerset::IncomingIndex) {
		let incoming = if let Some(pos) = self.incoming.iter().position(|i| i.incoming_id == index) {
			...
			error!(target: "sub-libp2p-foreign", "PSM => Reject({:?}): Invalid index", index);
			return
		};

		if !incoming.alive {
			error!(target: "sub-libp2p-foreign", "PSM => Reject({:?}, {:?}): Obsolete incoming, \
		...
		debug!(target: "sub-libp2p-foreign", "PSM => Reject({:?}, {:?}): Rejecting connection through \
			{:?}", index, incoming.peer_id, connected_point);
		debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Disable", incoming.peer_id);
		self.events.push(NetworkBehaviourAction::SendEvent {
			peer_id: incoming.peer_id,
			event: CustomProtoHandlerIn::Disable,
		});
	...
	TMessage: CustomMessage,
{
	...
	fn inject_connected(&mut self, peer_id: PeerId, connected_point: ConnectedPoint) {
		match (self.peers.entry(peer_id.clone()).or_insert(PeerState::Poisoned), connected_point) {
			...
			(st @ &mut PeerState::PendingRequest { .. }, connected_point) => {
				debug!(target: "sub-libp2p-foreign", "Libp2p => Connected({:?}): Connection \
					requested by PSM (through {:?})", peer_id, connected_point
				);
				debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Enable", peer_id);
				self.events.push(NetworkBehaviourAction::SendEvent {
					peer_id: peer_id.clone(),
					event: CustomProtoHandlerIn::Enable(connected_point.clone().into()),
				});
			...
			(st @ &mut PeerState::Banned { .. }, connected_point @ ConnectedPoint::Listener { .. }) => {
				...
				self.next_incoming_index.0 = match self.next_incoming_index.0.checked_add(1) {
					Some(v) => v,
					None => {
						error!(target: "sub-libp2p-foreign", "Overflow in next_incoming_index");
						return
					}
				};
				debug!(target: "sub-libp2p-foreign", "Libp2p => Connected({:?}): Incoming connection",
					peer_id);
				debug!(target: "sub-libp2p-foreign", "PSM <= Incoming({:?}, {:?}): Through {:?}",
			...
			(st @ &mut PeerState::Banned { .. }, connected_point) => {
				...
				debug!(target: "sub-libp2p-foreign", "Libp2p => Connected({:?}): Requested by something \
			...
			st => {
				...
				error!(target: "sub-libp2p-foreign", "Received inject_connected for \
	...
	fn inject_disconnected(&mut self, peer_id: &PeerId, endpoint: ConnectedPoint) {
		match self.peers.remove(peer_id) {
			...
			Some(PeerState::Banned { .. }) =>
				// This is a serious bug either in this state machine or in libp2p.
				error!(target: "sub-libp2p-foreign", "Received inject_disconnected for non-connected \
					node {:?}", peer_id),

			Some(PeerState::Disabled { open, banned_until, .. }) => {
				debug!(target: "sub-libp2p-foreign", "Libp2p => Disconnected({:?}): Was disabled \
					(through {:?})", peer_id, endpoint);
				if let Some(until) = banned_until {
					self.peers.insert(peer_id.clone(), PeerState::Banned { until });
				}
				if open {
					debug!(target: "sub-libp2p-foreign", "External API <= Closed({:?})", peer_id);
			...
			Some(PeerState::Enabled { open, .. }) => {
				debug!(target: "sub-libp2p-foreign", "Libp2p => Disconnected({:?}): Was enabled \
					(through {:?})", peer_id, endpoint);
				debug!(target: "sub-libp2p-foreign", "PSM <= Dropped({:?})", peer_id);
				self.peerset.dropped(peer_id.clone());
			...
			Some(PeerState::Incoming { .. }) => {
				if let Some(state) = self.incoming.iter_mut().find(|i| i.peer_id == *peer_id) {
					debug!(target: "sub-libp2p-foreign", "Libp2p => Disconnected({:?}): Was in incoming \
						mode (id {:?}, through {:?})", peer_id, state.incoming_id, endpoint);
					state.alive = false;
				} else {
					error!(target: "sub-libp2p-foreign", "State mismatch in libp2p: no entry in incoming \
	...
	fn inject_addr_reach_failure(&mut self, peer_id: Option<&PeerId>, addr: &Multiaddr, error: &dyn error::Error) {
		trace!(target: "sub-libp2p-foreign", "Libp2p => Reach failure for {:?} through {:?}: {:?}", peer_id, addr, error);
	}
	...
	fn inject_dial_failure(&mut self, peer_id: &PeerId) {
		if let Entry::Occupied(mut entry) = self.peers.entry(peer_id.clone()) {
			match mem::replace(entry.get_mut(), PeerState::Poisoned) {
				...
				st @ PeerState::Banned { .. } => {
					trace!(target: "sub-libp2p-foreign", "Libp2p => Dial failure for {:?}", peer_id);
					*entry.into_mut() = st;
				},

				// "Basic" situation: we failed to reach a node that the peerset requested.
				PeerState::Requested | PeerState::PendingRequest { .. } => {
					debug!(target: "sub-libp2p-foreign", "Libp2p => Dial failure for {:?}", peer_id);
	...
	fn inject_node_event(
		...
		match event {
			CustomProtoHandlerOut::CustomProtocolClosed { reason } => {
				debug!(target: "sub-libp2p-foreign", "Handler({:?}) => Closed: {}", source, reason);

				let mut entry = if let Entry::Occupied(entry) = self.peers.entry(source.clone()) {
					entry
				} else {
					error!(target: "sub-libp2p-foreign", "State mismatch in the custom protos handler");
					return
				};

				debug!(target: "sub-libp2p-foreign", "External API <= Closed({:?})", source);
				let event = CustomProtoOut::CustomProtocolClosed {
					reason,
				...
				match mem::replace(entry.get_mut(), PeerState::Poisoned) {
					PeerState::Enabled { open, connected_point } => {
						...
						debug!(target: "sub-libp2p-foreign", "PSM <= Dropped({:?})", source);
						self.peerset.dropped(source.clone());

						debug!(target: "sub-libp2p-foreign", "Handler({:?}) <= Disable", source);
						self.events.push(NetworkBehaviourAction::SendEvent {
							peer_id: source.clone(),
							event: CustomProtoHandlerIn::Disable,
						});
					...
					_ => error!(target: "sub-libp2p-foreign", "State mismatch in the custom protos handler"),
				}
			}

			CustomProtoHandlerOut::CustomProtocolOpen { version } => {
				debug!(target: "sub-libp2p-foreign", "Handler({:?}) => Open: version {:?}", source, version);
				...
				debug!(target: "sub-libp2p-foreign", "External API <= Open({:?})", source);
			...
			CustomProtoHandlerOut::CustomMessage { message } => {
				...
				trace!(target: "sub-libp2p-foreign", "Handler({:?}) => Message", source);
				trace!(target: "sub-libp2p-foreign", "External API <= Message({:?})", source);
