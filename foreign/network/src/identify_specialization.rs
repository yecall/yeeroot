...
use crate::{IdentifySpecialization, PeerId, IdentifyInfo};
use regex::Regex;
use log::debug;

const USERAGENT_SHARD: &str = "shard";

pub struct ForeignIdentifySpecialization {
    protocol_version: String,
    shard_num: u16,
    shard_re: Regex,
}

impl ForeignIdentifySpecialization {
    pub fn new(protocol_version: String, shard_num: u16) -> Self {
        let shard_re = Regex::new(&format!(r"{}/(\d+)", USERAGENT_SHARD)).unwrap();

        ForeignIdentifySpecialization {
            protocol_version,
            shard_num,
            shard_re,
        }
...
impl IdentifySpecialization for ForeignIdentifySpecialization {
