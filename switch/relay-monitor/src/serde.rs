...
pub trait SerdeHex: Sized {
		...
		S: Serializer,
	{
		...
		match Self::uint_size() {
			Some(_size) => impl_serde::serialize::serialize_uint(bytes.as_slice(), serializer),
			_ => impl_serde::serialize::serialize(bytes.as_slice(), serializer),
		}
	}
		...
		D: Deserializer<'de>,
	{
		...
		let bytes = match Self::uint_size() {
			Some(size) => impl_serde::serialize::deserialize_check_len(
				deserializer,
				impl_serde::serialize::ExpectedLen::Between(0, size),
			)?,
			_ => impl_serde::serialize::deserialize(deserializer)?,
		};
